                ...
                VStack(spacing: 0) {
                    if context.viewState.isLoading {
                        ProgressView()
                            ...
                            .padding()
                    } else if context.viewState.hasError {
                        VStack(spacing: 16) {
                            Text(L10n.commonSomethingWentWrongMessage)
...
struct RoomMessageSearchScreen_Previews: PreviewProvider, TestablePreview {
    ...
    static let errorViewModel: RoomMessageSearchScreenViewModel = {
        let searchProxy = RoomMessageSearchProxyMock()
        searchProxy.loadNextResultsReturnValue = .failure(.eventNotFound)
        let roomProxy = JoinedRoomProxyMock(.init())
        roomProxy.messageSearchProxyQueryReturnValue = searchProxy
    ...
    static var previews: some View {
        ...
        ElementNavigationStack {
            RoomMessageSearchScreen(context: errorViewModel.context)
        }
        .previewDisplayName("Error")
        .snapshotPreferences(expect: errorViewModel.context.observe(\.viewState.hasError))
    }
}
